...
describe('SplitMix 64', (): void => {
  ...
  it('should produce deterministic sequences for the same seed', (): void => {
    let a: SplitMix64 = splitMix64({ seed }).next(),
      b: SplitMix64 = splitMix64({ seed }).next()
  ...
  describe('canonical vectors', (): void => {
    describe.each(
      ...
      let prng: SplitMix64 = splitMix64({
        seed: BigInt(seed),
      }).next()
  ...
  describe('immutability', (): void => {
    it('should returns a new instance on next()', (): void => {
      const r0: SplitMix64<undefined> = splitMix64({ seed }),
        r1: SplitMix64 = r0.next()
  ...
  describe('jump', (): void => {
    it('should match next() for a single forward step', (): void => {
      const prng: SplitMix64<undefined> = splitMix64({ seed }),
        viaJump: SplitMix64 = prng.jump(1),
        viaNext: SplitMix64 = prng.next()
    ...
    it('should allow a walker to catch up to a jumper', (): void => {
      const base: SplitMix64<undefined> = splitMix64({ seed }),
        steps = 5,
        jumper: SplitMix64 = base.jump(steps)

      let walker: SplitMix64 = base.next()
    ...
    it('should allow a backward walker to catch up to a backward jumper', (): void => {
      const base: SplitMix64<undefined> = splitMix64({ seed }),
        steps = 100,
        start: SplitMix64 = base.jump(steps),
        jumper: SplitMix64 = start.jump(-steps)

      let walker: SplitMix64 = start.back()
    ...
    it('should walk back and forth between neighboring states', (): void => {
      const base: SplitMix64<undefined> = splitMix64({ seed }),
        forward: SplitMix64 = base.next(),
        backward: SplitMix64 = forward.back(),
        forwardAgain: SplitMix64 = backward.next()
    ...
    it('should support full back-and-forth traversal via jump()', (): void => {
      const base: SplitMix64<undefined> = splitMix64({ seed }),
        steps = 10,
        forward: SplitMix64 = base.jump(steps),
        backAgain: SplitMix64 = forward.jump(-steps),
        forwardAgain: SplitMix64 = backAgain.jump(steps)
